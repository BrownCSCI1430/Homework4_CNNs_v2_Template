...
\]
where $\mu_{n,c}$ and $\sigma_{n,c}^2$ are the mean and variance computed over the spatial dimensions $(H \times W)$ of image $n$, channel $c$ only.
...
\]
where $\mu_c$ and $\sigma_c^2$ are computed over all $B$ images and all spatial positions $(H \times W)$ for channel $c$. Then, define learnable parameters $\gamma_c$ and $\beta_c$ that produce an affine transform to rescale and reshift the batch distribution per channel. $\epsilon$ is a small constant to stop div.~by zero.
\end{question}

...
\item Assume discrete integer shifts on an integer grid and ignore boundary padding artifacts.
\end{itemize}
...
\[
\hat{x}_{n,c} = \frac{x_{n,c} - \mu_{n,c}}{\sqrt{\sigma_{n,c}^2 + \epsilon}}, \qquad y_{n,c} = \gamma_c \hat{x}_{n,c} + \beta_c
\]
where $\mu_{n,c}$ and $\sigma_{n,c}^2$ are the mean and variance computed over the spatial dimensions $(H \times W)$ of image $n$, channel $c$ only. $\gamma_c$ and $\beta_c$ are learnable parameters per channel.
...
\]
where $\mu_c$ and $\sigma_c^2$ are computed over all $B$ images and all spatial positions $(H \times W)$ for channel $c$. $\gamma_c$ and $\beta_c$ are learnable parameters. $\epsilon$ is a small constant to stop div.~by zero.
\end{question}
